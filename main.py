...
def make_chess_db():
    ...
    with open('matches_df', 'wb') as handle:
...
def make_states():
    ...
    # load in the db
    df = pickle.load(open('matches_df', 'rb'))
    ...
    for k, match in enumerate(df['moves']):
        ...
        if not bad_move:
            with open("all_states", "a+") as myfile:
                myfile.write(str(states) + '\n')
            with open("all_n_moves", "a+") as myfile:
                myfile.write(str(n_possible_moves) + '\n')

        else:
            with open("remove_matches", "a") as myfile:
                myfile.write(str(k) + '\n')
...
def refine_df():
    ...
    remove_matches = []
    with open('remove_matches') as fp:
        for line in fp:
            remove = int(re.sub('\n', '', line))
    ...
    # pickle to save the resulting chess db
    with open('matches_df_refined', 'wb') as handle:
        pickle.dump(df, handle)
...
def calculate_bdm():
    ...
    with open('all_states') as fp:
        ...
        for line in fp:
            ...
            with open("bdms_white_all", "a+") as myfile:
                myfile.write(str(bdms_white) + '\n')
            with open("bdms_black_all", "a+") as myfile:
...
def plot_results():
    ...
    # load in the db
    df = pickle.load(open('matches_df_refined', 'rb'))
    ...
    # plot a line
    with open('bdms_white_all') as fp:
        for i, line in enumerate(fp):
            line = re.sub('\n', '', line)
            line = ast.literal_eval(line)
            y = sum(line)/len(line)
            x = df.iloc[[i]]['white_elos']
            ys.append(y)
            xs.append(x)

    ax = sns.scatterplot(x=xs, y=ys, linewidth=0, s=1, alpha=0.7, legend=False)
    plt.xlabel('White ELO')
    plt.ylabel('Mean BDM')
    plt.show()

    with open('bdms_black_all') as fp:
        for i, line in enumerate(fp):
            line = re.sub('\n', '', line)
            line = ast.literal_eval(line)
            y = sum(line)/len(line)
            x = df.iloc[[i]]['black_elos']
            ys.append(y)
            xs.append(x)
